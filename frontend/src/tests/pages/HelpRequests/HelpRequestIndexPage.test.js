...
describe("HelpRequestIndexPage tests", () => {
    ...
    test("renders three helpRequests correctly for regular user", async () => {
        ...
        const requesterEmail = screen.getByText("other@ucsb.edu");
        expect(requesterEmail).toBeInTheDocument();

        const teamId = screen.getByText("team02");
        expect(teamId).toBeInTheDocument();

        const table = screen.getByText("table2");
        expect(table).toBeInTheDocument();

        const time = screen.getByText("2021-01-02T12:00");
        expect(time).toBeInTheDocument();

        const explain = screen.getByText("posgres");
